...
class LoginGUI extends JFrame implements ActionListener 
{
   ...
   /* Panel */
   JPanel basePanel;//12-11 변경= new JPanel(new BorderLayout());
   ...
   public LoginGUI(Socket socket,String ip,int portNum) 
   {
         ...
         //12-11 추가 뒷 배경 설정. 상대경로로 설정이 잘안되서 절대경로로 이미지 얻음.
         ...
         JPanel basePanel = new JPanel(new BorderLayout())
         {
            ...
            //12-11추가 로고 띄우고 그 밑에 프로그램 이름 띄우기. 절대경로로 이미지얻음.
         ...
         /* Panel 추가 작업 */
         //setContentPane(basePanel); //12-11 제거
         //12-11아래 setBackground값 변경.
         ...
         setSize(500, 600);
         //12-11제거 setLocationRelativeTo(null);
         setLocation(555, 200);//12-11추가
         setVisible(true);
         setResizable(false);
      ...
      try 
      {
         ...
         if (e.getSource() == loginBtn)
         {
            ...
            { 
               ...
               new Client_mainGUI(socket.getInetAddress(),socket.getPort(),u,d);
           ...
           {           
                 ...
                 if (SwingUtilities.isRightMouseButton(me)&& !UserList.isSelectionEmpty()&& UserList.locationToIndex(me.getPoint())== UserList.getSelectedIndex()) 
                 {
                      ...
                      for(int j=0;j<sb1.UserList.size();j++) //추가
                      {
                       ...
                       user u = null;
                       String id=(String)UserList.getSelectedValue(); //12-11추가 클릭된유저의 id를 저장
                       ...
                       {//list를 마우스 우클릭할시에 클릭된 사람의 정보 얻어서 클릭된 사람에 대한 리스너 생성.
                          ...
                          info.addActionListener(new showInfo(u,CB,d));
...
class Client_mainGUI extends JFrame implements ActionListener, KeyListener 
{
   private static final long serialVersionUID = 1L;
   ...
   public Client_mainGUI(InetAddress ip, int portNum,user u,Data d) throws Exception
   {
      ...
      */
      ...
      String[] savedKey= {"    데이터 조회일시:", "    일일확진:", "    일일사망:"}; //1212변경 띄어쓰기 추가.
      ...
      JButton coronaButton=new JButton("구체적인 코로나 정보 확인"); //1212add 
      coronaButton.setHorizontalAlignment(SwingConstants.CENTER); //1212add
      coronaButton.setVerticalAlignment(SwingConstants.CENTER); //1212add
      coronaButton.setBounds(400, 700, 400, 100); //1212 add
      //1212추가 코로나 정보 url 열기
      ...
      basePanel.add(coronaButton); //1212 추가 
      setVisible(false);
   ...
   {
      int result = JOptionPane.showConfirmDialog(this, message ,"확인창",JOptionPane.YES_NO_OPTION);
   ...
   public void AppendUserList(ArrayList<user> FriendList) throws Exception
   {
     ...
     String id;
      ...
      // UserList.setBackground(Color.white); //1212변경.
      // UserList.setBounds(200, 20,400,300); //1212변경
      ...
      //팝업 메뉴 만들기
      popupMenu = new JPopupMenu();
      JMenuItem jm1= new JMenuItem("친구 정보 보기");
      JMenuItem jm2= new JMenuItem("1:1 채팅하기");
      JMenuItem jm3= new JMenuItem("파일 보내기");
      JMenuItem jm4= new JMenuItem("1:1 게임 시작하기"); 
     
      // jm1.setBackground(new Color(255, 0, 0, 0)); //1212변경.
      // jm2.setBackground(new Color(255, 0, 0, 0)); //1212변경.
      // jm3.setBackground(new Color(255, 0, 0, 0)); //1212변경.
     //  jm4.setBackground(new Color(255, 0, 0, 0)); //1212변경.
      ...
      UserList.addMouseListener(new MouseAdapter()
      {
            public void mouseClicked(MouseEvent me) 
            {
              ...
              if (SwingUtilities.isRightMouseButton(me)&& !UserList.isSelectionEmpty() && UserList.locationToIndex(me.getPoint())== UserList.getSelectedIndex()) 
              {
                 popupMenu.add(new JMenuItem(UserList.getSelectedValue().replace("online", "").replace("offline", "").trim() +"친구 정보 보기"));
                  popupMenu.show(UserList, me.getX(), me.getY());            
            
                  for(int j=0;j<sb1.UserList.size();j++) //추가
                    {
                       if(sb1.UserList.get(j).id.equals(UserList.getSelectedValue()))//유저리스트에서 해당유저의i찾기.
                       {
                          u=sb1.UserList.get(j);
                         break;
                       }
                  ...
                  //친구정보보기 메뉴를 클릭했을때
                  jm1.addActionListener(new showInfo(u,CB,d));
              }
